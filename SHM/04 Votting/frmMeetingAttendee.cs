...
namespace SHM
{
    public partial class frmMeetingAttendee : Form
    {
        #region Fields
        ...
        public frmMeetingAttendee()
        {
            InitializeComponent();
        }

        private void frmMeetingAttendee_Load(object sender, EventArgs e)
        {
        ...
        private void deleteToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DataObj.OpenConnection();
                string cmdText = "delete from tblMeetingAttendee where sessionId=" + this._SessionId + " and SHId=" + this._SHId;
        ...
        private void GetMeetingAtendee(int SessionId)
        {
            ...
            try
            {
                ...
                DataObj.FillData(this._tblShareholderDetail, "SELECT a.SHId, a.RegCode, a.FullName,b.SubscribedShareNoModif as ShareNo,b.SubscribedShareAmtModif as ShareAmnt FROM tblShareholderDetail as a inner join  tblShareRegBook as b on a.SHId= b.SHId WHERE a.SHStatus=1 and a.SHId NOT IN (SELECT SHId FROM tblMeetingAttendee WHERE SessionId in(select SessionId from tblMeetingSession where IsActive=1))");
                ...
                DataObj.FillData(this._tblMeetingAttendee, "SELECT  SessionId, SHId, RegCode,ShareNo,ShareAmnt FROM tblMeetingAttendee   WHERE SessionId in(select SessionId from tblMeetingSession where IsActive=1)");
            }
            catch (Exception Ex)
            {
                AppMessageBox.ShowExclamation(Ex.Message);
        ...
        private void DeleteAttendee(int sessionId, int SHId)
        {
            ...
            {
                DataObj.OpenConnection();
                cmdText = "delete from tblMeetingAttendee where sessionId=" + this._SessionId + " and SHId=" + this._SHId;
                if (DataObj.Execute(cmdText))
                {
                    this._SHId = 0;
                   // AppMessageBox.ShowInformation("the attendee  has been deleted sucessfully");
